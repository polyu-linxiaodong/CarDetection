...
class DecodeBox():
    ...
    def frcnn_correct_boxes(self, box_xy, box_wh, input_shape, image_shape):
        #-----------------------------------------------------------------#
        #   Putting the y-axis first is convenient for multiplying the predicted box with the image width and height
        #-----------------------------------------------------------------#
        box_yx = box_xy[..., ::-1]
        box_hw = box_wh[..., ::-1]
    ...
    def forward(self, roi_cls_locs, roi_scores, rois, image_shape, input_shape, nms_iou = 0.3, confidence = 0.5):
        ...
        #----------------------------------------------------------------------------------------------------------------#
        #   Process each image. Since we only input one image in predict.py, for i in range(len(mbox_loc)) only runs once
        #----------------------------------------------------------------------------------------------------------------#
        for i in range(bs):
            #----------------------------------------------------------#
            ...
            #----------------------------------------------------------#
            #   The first dimension is the number of proposals, the second dimension is each class
            #   The third dimension is the adjustment parameters for the corresponding class
            #----------------------------------------------------------#
            roi_cls_loc = roi_cls_loc.view([-1, self.num_classes, 4])

            #-------------------------------------------------------------#
            #   Use the prediction results of the classifier network to adjust the proposals to obtain the predicted boxes
            ...
            #-------------------------------------------------------------#
            #   Normalize the predicted boxes to be between 0 and 1
            #-------------------------------------------------------------#
            ...
            for c in range(1, self.num_classes):
                #--------------------------------#
                #   Extract the confidence of all boxes belonging to this class
                ...
                if len(c_confs[c_confs_m]) > 0:
                    #-----------------------------------------#
                    #   Extract boxes with scores higher than confidence
                    #-----------------------------------------#
                    ...
                    #-----------------------------------------#
                    #   Extract the content with better results in non-maximum suppression
                    #-----------------------------------------#
