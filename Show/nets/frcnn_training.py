...
class AnchorTargetCreator(object):
    ...
    def _calc_ious(self, anchor, bbox):
        #   The shape of ious is [num_anchors, num_gt]
        ious = bbox_iou(anchor, bbox)

        if len(bbox)==0:
            return np.zeros(len(anchor), np.int32), np.zeros(len(anchor)), np.zeros(len(bbox))
        #   Get the most corresponding gt box for each anchor [num_anchors, ]
        argmax_ious = ious.argmax(axis=1)
        #   Get the IoU of the most corresponding gt box for each anchor [num_anchors, ]
        max_ious = np.max(ious, axis=1)
        #   Get the most corresponding anchor for each gt box [num_gt, ]
        gt_argmax_ious = ious.argmax(axis=0)
        #   Ensure that each gt box has a corresponding anchor
    ...
    def _create_label(self, anchor, bbox):
        ...
        #   Set as negative sample if IoU is less than threshold
        #   Set as positive sample if IoU is greater than threshold
        #   Ensure each gt box has at least one corresponding anchor
        label[max_ious < self.neg_iou_thresh] = 0
        label[max_ious >= self.pos_iou_thresh] = 1
        if len(gt_argmax_ious)>0:
            label[gt_argmax_ious] = 1
        #   Limit the number of positive samples to 128 if it exceeds
        ...
        #   Balance positive and negative samples, keep total number as 256
...
class ProposalTargetCreator(object):
    ...
    def __call__(self, roi, bbox, label, loc_normalize_std=(0.1, 0.1, 0.2, 0.2)):
        ...
        #   calculate IoU between each roi and gt box
        ...
        if len(bbox)==0:
            ...
            #   get the most corresponding gt box for each roi
...
class FasterRCNNTrainer(nn.Module):
    ...
    def forward(self, imgs, bboxes, labels, scale):
        ...
        #   use the feature map to get the anchor, rpn_loc, rpn_score, roi, roi_indices
        ...
        for i in range(n):
            ...
            #   calculate the loss of rpn network
            # -------------------------------------------------- #
        ...
        for i in range(n):
            #   according to the roi_indices, get the corresponding roi_cls_loc, roi_score, gt_roi_loc, gt_roi_label
            ...
            #  calculate the loss of roi network
